    ...
    <div className="split-view-container">
      <div className="left-column">
        ...
        {filteredPatients.map(p => (
          <div key={p.id} onClick={() => { setSelectedPatientId(p.id); setEditingVisitId(null); }}
           className={`patient-row-card ${selectedPatientId === p.id ? 'patient-row-card-selected' : ''}`}>
            <strong>{p.name}</strong>
            <p style={{ margin: '5px 0 0 0', color: 'gray' }}>Age: {p.age}</p>
          </div>
        ))}
      </div>
      <div className="right-column">
        {selectedPatient ? (
          <div>
            ...
            {selectedPatient.visits.length === 0 ? <p style={{ color: 'gray' }}>
              No medical history logs found.</p> : selectedPatient.visits.map(visit => (
              <div key={visit.visitId} className="visit-history-box">
                {editingVisitId === visit.visitId ? (
                  <div>
                    <input type="text" className="edit-input-field" value={editDiagnosis} 
                    onChange={(e) => setEditDiagnosis(e.target.value)} />
                    <textarea className="edit-input-field" value={editNotes} 
                    onChange={(e) => setEditNotes(e.target.value)} />

                    <div className="action-btn-row" style={{ marginTop: '10px' }}>
                      <button className="btn-edit-action" style={{ backgroundColor: '#10b981', color: 'white' }} 
                      onClick={() => handleSave(visit.visitId)}>Save</button>
                  ...
                  <div>
                    ...
                    <div className="medicine-tag-box">
                      ...
                      {visit.medicines?.map((med, i) => <div key={i} style={{ fontSize: '13px' }}>
                        • {med.name} - {med.dosage} ({med.time})</div>)}
                    </div>
                    <div className="action-btn-row">
                      <button className="btn-edit-action" 
                      onClick={() => { setEditingVisitId(visit.visitId); setEditDiagnosis(visit.diagnosis); setEditNotes(visit.notes); }}>✏️ Edit</button>
                      <button className="btn-delete-action" 
                      onClick={() => handleDelete(visit.visitId)}>🗑️ Delete</button>
                    </div>
                  </div>
                )}
              </div>
        ...
        ) : <p style={{ textAlign: 'center', color: 'gray', marginTop: '100px' }}>

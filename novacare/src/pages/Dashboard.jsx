...
function Dashboard() {
  const patients = useSelector(
    (state) => state.clinic.patients
  );
  const appointments = useSelector(
    (state) => state.clinic.appointments
  );

  const totalPatients = patients.length;
  let totalVisitsCount = 0;
  patients.forEach(p => { 
    totalVisitsCount += p.visits.length; 
  });
  ...
  return (
    <div>
      ...
      <div className="card-row">
        ...
        <div className="metrics-card" style={{ borderTopColor: '#10b981' }}>
          <h3 className="card-title">Visits Logged</h3>
          <p className="card-number" style={{ color: '#10b981' }}>{totalVisitsCount}</p>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginTop: '15px' }}>
            {appointments.map(app => (
              <div key={app.id} className="visit-history-box" style={{ margin: 0, borderLeft: '5px solid #0284c7' }}>
                ...
                <span style={{ float: 'right' }}>🗓️ {app.date}</span>

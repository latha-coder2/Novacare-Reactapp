      ...
      <div className="login-box" style={{ maxWidth: isRegisterMode && role === 'patient' ? '500px' : '400px' }}>
        ...
        <form onSubmit={handleAuthSubmit}>
          <div className="form-group">
            ...
            <input type="text" className="form-field" value={username} onChange={(e) => setUsername(e.target.value)} required />
          </div>
          <div className="form-group">
            <label className="form-label">Password</label>
            <input type="password" className="form-field" value={password} onChange={(e) => setPassword(e.target.value)} required />
          </div>
          ...
          {isRegisterMode && role === 'patient' && (
            <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', marginBottom: '15px' }}>
              <div className="form-group">
                ...
                <input type="text" className="form-field" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
              </div>
              <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                <input type="number" placeholder="Age" className="form-field" value={age} onChange={(e) => setAge(e.target.value)} required />
                <select className="form-field" value={gender} onChange={(e) => setGender(e.target.value)}>
        ...
        <p className="switch-auth-text">
          {isRegisterMode ? "Have an account? " : "New here? "}
          <span className="switch-auth-link" onClick={() => setIsRegisterMode(!isRegisterMode)}>
